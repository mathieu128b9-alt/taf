...
/*   Updated: 2025/12/11 08:46:02 by msuter           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fract_ol.h"

double	mandelbroot(t_all *all, double i,
	double mandel_c_re, double mandel_c_im)
{
	double	temp;
	double	z_re;
	double	z_im;
...
void	color_pixel(t_all *all, int offset_total, double i)
{
	...
	else
	{
		...
		all->g.addr[offset_total + 1] = all->f.g_1
			+ all->f.g_2 * cos(2 * N_PI * t + 2);
		all->g.addr[offset_total + 2] = all->f.b_1
			+ all->f.b_2 * cos(2 * N_PI * t + 4);
	}
}
